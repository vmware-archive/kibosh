...
func (r *repository) ClearCache() error {
	r.chartsCache = nil
	r.logger.Info("Cleared Cache for Broker Repository")
	return nil
}
func (r *repository) GetCharts() ([]*helm.MyChart, error) {
	if r.chartsCache == nil {
		...
		if chartExists {
			myChart, err := helm.NewChart(r.helmChartDir, false, r.privateRegistryServer, r.logger)
			if err != nil {
				return nil, err
			}
			...
			for _, fileInfo := range helmDirFiles {
				...
				if fileInfo.IsDir() {
					...
					if subdirChartExists {
						myChart, err := helm.NewChart(filepath.Join(subChartPath),false, r.privateRegistryServer, r.logger)
						if err != nil {
							return nil, err
						}
						charts = append(charts, myChart)
...
func (r *repository) SaveChart(path string) error {
	...
	chart, err := helm.NewChart(chartPath, false,r.privateRegistryServer, r.logger)
	if err != nil {
		return err
	}
